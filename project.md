...
**Persistence**: `localStorage["netprofit.v1"]` = `{coins, earned, muted, lv:{net,hold,engine}, paint, log[], order, wood, build, market, levSeen, trip}`. `keys` is `"drive"` or `"point"`, the keyboard feel, missing in old saves and read as drive. `market` is the fishmonger's pick of the day, a species index or -1 when there is none, missing in old saves and read as -1. `trip` is `{x, y, h, clock, hold[]}`: the boat's position and heading, the time of day and the hold. It is written on every save, when the tab is hidden, on pagehide and every five seconds while sailing, and restored on load, so a phone that discards the tab does not lose the haul (added 2026-09-17; legacy does not have it, and old saves without it load unchanged). Entity state is not saved.
